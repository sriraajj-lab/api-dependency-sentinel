...
export default function Workspace() {
  const { data, isLoading, error, refetch } = trpc.sentinel.pipelinePreview.useQuery();
    ...
    <div className="min-h-screen bg-[#101629] text-[#f7f8fc]">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#101629]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-[68px] max-w-[1560px] items-center justify-between px-5 lg:px-8">
          <div className="flex min-w-0 items-center gap-4"><a href="/" className="grid h-9 w-9 place-items-center rounded-lg border border-white/12 text-[#b8c0d5] hover:bg-white/8" aria-label="Back to landing page"><ArrowLeft size={17} /></a><div className="min-w-0"><div className="flex items-center gap-2"><span className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#f1c85b]">Pipeline evidence preview</span><span className="hidden h-1 w-1 rounded-full bg-[#697794] sm:block" /><span className="hidden font-mono-ui text-[10px] uppercase tracking-[.12em] text-[#8190ad] sm:inline">read-only fixture</span></div><h1 className="truncate text-[15px] font-bold tracking-[-.025em]">{data.workspace.repository}</h1></div></div>
          <div className="flex items-center gap-3"><a href="/workspace/live" className="hidden rounded-lg border border-[#6ab8ff]/25 px-3 py-2 font-mono-ui text-[10px] uppercase tracking-[.1em] text-[#9ccfff] hover:bg-[#6ab8ff]/10 sm:inline">Saved evidence</a><span className="hidden rounded-full border border-[#6ddaa7]/25 bg-[#6ddaa7]/10 px-2.5 py-1 font-mono-ui text-[9px] uppercase tracking-[.12em] text-[#8ce8bc] sm:inline">Risk map ready</span><button onClick={() => setMenuOpen((open) => !open)} className="inline-flex items-center gap-2 rounded-lg border border-white/12 px-3 py-2 text-[12px] font-semibold text-white hover:bg-white/8">Workspace <ChevronDown size={14} /></button></div>
        </div>
        {menuOpen && <div className="absolute right-5 top-[60px] z-50 w-[290px] rounded-xl border border-white/12 bg-[#1b2440] p-4 shadow-2xl lg:right-8"><div className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#f1c85b]">Pipeline boundary</div><p className="mt-2 text-[12px] leading-5 text-[#b8c0d5]">This reviewer record is generated from a controlled Stripe schema-diff and TypeScript AST fixture. Live provider polling and customer-repository scans are the next activation step.</p></div>}
      </header>
      ...
      <main className="mx-auto max-w-[1560px] px-5 py-6 lg:px-8 lg:py-8">
        <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#171f38] p-6 sm:p-8">
          ...
          <div className="relative grid gap-8 xl:grid-cols-[1.2fr_.8fr] xl:items-end"><div><div className="flex items-center gap-2 font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#f1c85b]"><Radar size={14} /> Integration risk map</div><h2 className="mt-4 max-w-2xl text-4xl font-bold leading-[.94] tracking-[-.06em] sm:text-5xl">Every finding should carry its proof.</h2><p className="mt-4 max-w-xl text-[14px] leading-6 text-[#b6c0d6]">This controlled pipeline fixture connects a Stripe schema diff to an exact TypeScript call site, commit, confidence score, and provenance chain for owner review.</p></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-2">{[[data.summary.monitoredProviders, "providers watched"], [data.summary.activeFindings, "active findings"], [`${data.summary.highestRisk}/100`, "highest risk"], [`${data.summary.confidence}%`, "match confidence"]].map(([value, label]) => <div key={String(label)} className="rounded-xl border border-white/10 bg-[#0f1528]/70 p-4"><div className="text-2xl font-bold tracking-[-.06em] text-white">{value}</div><div className="mt-1 font-mono-ui text-[9px] uppercase tracking-[.12em] text-[#8794b0]">{label}</div></div>)}</div></div>
        </section>
